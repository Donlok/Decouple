...
#	cont_b=60000.0
#	alph_0=.00691
#	return (A_0*np.exp(np.abs(cent-neig)/B_0)+alph_0)
#	return (A_0*np.exp(np.abs(cent)/B_0)+alph_0+(cont_b-np.abs(cent-neig))*cont_m)
...
#		if(np.abs((np.mean(np.abs(previous))-np.mean(np.abs(array))))/np.mean(np.abs(previous))<1e-7):
#			break
		i+=1
	i=0
	while(i<iterations):
...
#initial_array=60000.0*np.random.uniform(size=(size-4,size-4))
#initial_array=pad_zeros(initial_array)
#initial_array=pad_zeros(initial_array)
...
#		b=ref_table(init[i,j],init[i+1,j],S,C,A)*0.01
