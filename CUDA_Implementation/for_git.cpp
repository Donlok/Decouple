...
#include <limits>			//float rounding I think?
...
void print_line(long double*A, int size);
void print_array(long double**A, int col, int row);
void deconstruct_subarray(SubArray& B);
void array_to_subarray(long double** A, SubArray& B, int startcol, int startrow, int cols, int rows);
void check(int a);
void init_array(long double**& A, int col, int row);

//I think this removes the contents from subarray.values and allows for reutilization of the same subarray again.  we'll see...
void deconstruct_subarray(SubArray& B){
...
//this should deconstruct a line array and reinitialize the first vector to NULL
void deconstruct_line(long double*& A){
	delete A;
	A = NULL;
}

// this should deconstruct a float line and reinitialize the first vector to NULL
...
void deconstruct_array(long double **& A, int rows){
	int i = 0;
	while (i < rows){
		delete[] A[i];
		i++;
...
void deconstruct_tuple(std::tuple<long double**, int, int>& A){
	int i = 0;
...
void array_to_subarray(long double** A, SubArray& B, int startcol, int startrow, int cols, int rows){
	B.values = new long double*[rows];
	int m = 0;
	for (int i = startrow; i < (startrow + rows); i++){
		int n = 0;
		B.values[m] = new long double[cols];
...
// Places values from a subarray into a pre-existing long double**
// long double** must have sufficient size.  the function will never throw any errors but will not include data outside its initialized dimensions
void subarray_into_array(long double**& A, SubArray B, int rows, int cols){
...
//this may need some tweaking still to make the .csv readily convertable into a .fit or a numpyarray
void array_to_csv(long double** A, int col, int row, std::string fname){
	std::ofstream ofile;
	std::stringstream ss;
	std::string str;
	ofile.open(fname);
...
void array_diff_mag(long double**&A, long double**B, int col, int row){
	for (int i = 0; i < row; i++){
		for (int j = 0; j < col; j++){
...
std::tuple<long double**, int, int> csv_to_array(std::string fname, int imin, int imax, int jmin, int jmax){
	...
	i = 0;
	A = new long double*[imax - imin];
	while ((i < *max_size) && (i<(imax - imin))){
		int j = 0;
		A[i] = new long double[jmax - jmin];
...
// long double** A contains alpha data
// C contains 'contrast' data, just one look up dimension for alpha
// S contains 'signal' data, anohter lookup dimension for alpha
// This could be the bottleneck for large text files.
...
// initializes a line array of long doubles
void init_line(long double*&A, int size){
	A = new long double[size];
}

//initializes a two dimensional array of long doubles
void init_array(long double**&A, int col, int row){
	A = new long double*[row];
	for (int i = 0; i < row; i++){
		A[i] = new long double[col];
	}
}

//fills a line with values from offset counting upward
void fill_line(long double*&A, int size, int offset){
...
void fill_array(long double**&A, int col, int row, long double offset){
	for (int i = 0; i < row; i++){
		for (int j = 0; j < col; j++){
...
void fill_array_0s(long double**&A, int col, int row){
	for (int i = 0; i < row; i++){
		for (int j = 0; j < col; j++){
			A[i][j] = 0.0;
		}
...
void vectorize_array(long double**A, long double*&a, int col, int row){
	for (int i = 0; i < row; i++){
		for (int j = 0; j < col; j++){
			a[i*col + j] = A[i][j];
		}
...
void arrayize_vector(long double*a, long double**&A, int col, int row){
	for (int i = 0; i < row; i++){
		for (int j = 0; j < col; j++){
			A[i][j] = a[i*col + j];
		}
...
void simple_add(long double**&C, long double**A, long double**B, int col, int row){
	for (int i = 0; i < row; i++){
		for (int j = 0; j < col; j++){
			C[i][j] = A[i][j] + B[i][j];
		}
...
long double array_diff(long double**A, long double**B, int col, int row, int ignore){
	long double running_error = 0.0;
	for (int i = ignore; i < (row - ignore); i++){
		for (int j = ignore; j < (col - ignore); j++){
			running_error += abs(A[i][j] - B[i][j]);
		}
	}
	return running_error / (long double)((col - (2 * ignore))*(row - (2 * ignore)));
}
...
long double array_max_diff(long double**A, long double**B, int col, int row, int ignore){
...
void print_time(std::clock_t t2, std::clock_t t1){
	if (v){
		if ((t2 - t1 / (long double)(CLOCKS_PER_SEC / 1000)) < 1000){
			std::cout << (t2 - t1) / (long double)(CLOCKS_PER_SEC / 1000) << " ms\n";
		}
		else if (((t2 - t1) / (long double)(CLOCKS_PER_SEC)) > 60){
			std::cout << (long double)(t2 - t1) / (long double)(CLOCKS_PER_SEC * 60) << " min\n";
		}
		else {
			std::cout << (t2 - t1) / (long double)CLOCKS_PER_SEC << " s\n";
		}
	}
	bool a = true;
}
...
long double** cut_once(long double**& A, int& col, int& row){
	long double**B;
	int i = 0;
...
void cut_edges(long double**& A, int& col, int& row, int n){
	int k = 0;
	while (k < n){
		//		std::cout << k << std::endl;
...
//contains sanitation for first and last value cases but I don't remember it off the top of my head.
//it works in tandem with the other support functions though, I promise.
...
__device__ long double ref_table(long double Sin, long double Cin, float* Stab, float* Ctab, int Slen, int Clen, float* Atab){
	...
	long double for_ret = (xlw*ylw)*Atab[fir] + (xlw*yhw)*Atab[sec] + (xhw*ylw)*Atab[thi] + (xhw*yhw)*Atab[fo];
	return for_ret;
}
...
//limited size is 262,144 simultaneous pixels (512x512)
__global__ void decoupleKernel(long double *measured, long double *updated, const int col, const int row, float*Stab, float* Ctab, float*Atab, int Slen, int Clen){
	...
	long double coup_out = (coef_1 + coef_2 + coef_3 + coef_4)*0.01*updated[base];
...
cudaError_t decWithCuda(long double *&image, unsigned int col, unsigned int row, float*Stab, float*Ctab, float*Atab, unsigned int Slen, unsigned int Clen,
							cudaError_t cudaStatus, long double *&dev_measured, long double *&dev_updated, float *&dev_Stab, float *&dev_Ctab, float *&dev_Atab)
{
	unsigned int size = col*row;	// total length of the array

	cudaStatus = cudaMemcpy(dev_measured, image, size * sizeof(long double), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMemcpy failed!");
		goto Error;
	}
	cudaStatus = cudaMemcpy(dev_updated, image, size * sizeof(long double), cudaMemcpyHostToDevice);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMemcpy failed!");
		goto Error;
	}
	...
	cudaStatus = cudaMemcpy(image, dev_updated, size * sizeof(long double), cudaMemcpyDeviceToHost);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMemcpy failed!");
		goto Error;
	}
...
int main()
{
	...
	if (memory_check){
		...
		while (i < 1000){
			std::tuple<long double**, int, int> testCSV;
	...
	//This is the file that contains the look-up table formatted in a particular way.  At the moment it is generated by a python script.
	...
	std::tuple<long double**, int, int> CSV;
	...
	//Do not be tempted to do allocation for each kernel call; in my experience that results in synchronization failure after ~50 calls
	long double *dev_measured = NULL;		// device vector storing the input image to be used in calculations
	long double *dev_updated = NULL;		// device vector to update and ultimately contain the restored image
	...
	cudaStatus = cudaMalloc((void**)&dev_measured, size * sizeof(long double));
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
	}

	cudaStatus = cudaMalloc((void**)&dev_updated, size * sizeof(long double));
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed!");
	}
	...
	while (!in_cou.empty() && (!(cycle == term_cycle))){
		...
		float*vecA = NULL;
		long double*vecCSV = NULL;
		long double** result = NULL;
		int boxes = 0;
		...
		while (w < 1){
			if (*max_size > 512){
				...
				for (int p = 480; (p + 64) < ccol; p = p + 512){
					int q = 0;
					SubArray hat;
				...
				for (int q = 480; (q + 64) < crow; q = q + 512){
					int p = 0;
					SubArray hat;
			...
			int max_size_int;
			if (*max_size > 512){ max_size_int = 512; }
			...
			for (int p = 0; p + max_size_int <= crow; p = p + max_size_int){
				for (int q = 0; q + max_size_int <= ccol; q = q + max_size_int){
					...
					if (cudaStatus != cudaSuccess) {
						//						std::cout << "CUDA Error code of" << cudaStatus << std::endl;
						fprintf(stderr, "decWithCuda failed!");
						return 1;
					}
					boxes++;
...
// At this point the size is set to autoconfigure with the constraint that the initial constant int max_size be declared to be the array size.  Also only square arrays right now... sorry bro...
